...
网络诊断增加绑定物理网卡的外网探测，使用固定的 Cloudflare 公共 DNS-over-HTTPS 目标查询 example.com 并验证 HTTPS 证书，不携带账号密码。充值时间提示访问 Cloudflare、阿里云的 NTP 服务，不会修改设备时间；时间查询失败或时段不匹配均不会在本机阻拦充值。
