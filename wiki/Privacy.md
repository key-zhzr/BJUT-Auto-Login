...
临时填写的密码在切换应用时保留于当前表单内存并隐藏显示，不写入草稿文件；取消或提交后清空。月末余额检查只持久保存按账号散列区分的检查时间与日期，用于避免重复查询，不保存余额或密码。
