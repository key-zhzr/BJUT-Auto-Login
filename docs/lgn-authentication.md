...
macOS 的轻量 IP 轮询和完整网络观察共用同一选择器：优先校园有线，再选择 Wi-Fi。两者同时连接时，Wi-Fi 地址变化不会把当前校园有线地址替换成轮询基准；仅在实际选中 Wi-Fi 时读取 SSID/BSSID。

有线 IPv6 诊断读取当前接口的 DNS 和搜索域。保留 BJUT IPv6 地址、缺少 IPv6 DNS、搜索域为 `proxy.bjut.edu.cn` 的组合会被标注为与用户报告的异常配置一致，但不会据此断言 IPv6 一定不可达。lgn6 地址发现失败或命中该组合时，macOS 诊断页提供“重启有线适配器”。用户点击后重新核对网卡、IPv4 和异常状态，经系统管理员授权停用并重新启用对应服务，再刷新诊断。地址变化会中止操作，重新启用失败时会执行恢复尝试。测试只运行模拟网络命令，不重启真实设备。
