...
- 校园网计费中心：直接查看余额、流量、在线会话和各类账单，支持完整 CSV 导出，并可在二次确认后办理停复机、套餐预约、消费保护、设备绑定及安全设置。
...
本应用的账号密码仅保存在设备的安全凭据存储中：macOS 使用应用私有目录内、权限限制为当前用户的 AES-GCM 加密文件，Windows 使用 Credential Manager，Linux 使用 Secret Service，Android 使用 Android Keystore；常规 `config.json` 不包含密码。导出配置时会要求你设置独立密码并采用 AES-GCM 加密。应用不会向第三方服务器上传校园网密码，认证请求仅发送至北京工业大学校园网认证网关或校内计费系统 `jfself.bjut.edu.cn`。

计费中心每次访问均建立临时 HTTPS 会话，并在读取或操作完成后主动注销；密码、密码保护答案和会话安全令牌不会写入运行日志。账单 CSV 可能包含 IP、MAC 和使用记录等个人网络信息，请妥善保管导出文件。
